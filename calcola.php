...
/*
 * Progetto "Codice Fiscale" - Componente di calcolo
...
if (empty($_REQUEST)) {
    echo "<p>Benvenuto! Per funzionare, l'algoritmo necessita, tramite GET o POST, dei seguenti parametri.<br/><br/>";
    echo "Nome=Mario&Cognome=Rossi&Sesso=M&Anno=1980&Mese=01&Giorno=01&Comune=Milano</p>";
    return;
}
...
$trovato = false;
if (empty($property['nam']) || empty($property['cogn']) || strlen($property['mf']) != 1 || strlen($property['yy']) != 4 || strlen($property['mm']) != 2 || strlen($property['dd']) != 2 || empty($property['cm']) || !($property['mf'] == 'M' || $property['mf'] == 'F')) {
    $continua = false;
} else {
    $continua = true;
    if ($property['mf'] == 'M')
...
$myfile = fopen("comuni.csv", "r") or die ("Errore interno!");
while (!feof($myfile)) {
    if ($continua == false)
        break;
    $linea = fgets($myfile);
...
if ($continua == false || $trovato == false) {
    echo "Impossibile effettuare il calcolo!";
    return;
}

$codiceFiscale = new CodiceFiscale($property);

echo $codiceFiscale->codice;
